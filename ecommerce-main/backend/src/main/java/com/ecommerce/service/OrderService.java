...
public class OrderService {
	...
	public OrderDTO convertToOrderDTO(Order order) {
		List<OrderItemDTO> item = order.getOrderItems().stream().map(oi -> new OrderItemDTO(oi.getOrderItemId(),
				oi.getProductName(), oi.getProductImageUrl(), oi.getProductPrice(), oi.getQuantity(), oi.getTotalPrice()))
				.collect(Collectors.toList());
		User user=order.getUser();
		return new OrderDTO(order.getOrderId(), order.getOrderDate(), order.getTotalPrice(), order.getOrderStatus(),
				order.getShippingAddress(), order.getPaymentStatus(), user.getId(), user.getName(),user.getEmail(), item);
	}
